...
def server_loop(local_host, local_port, remote_host,  remote_port, receive_first):
    ...
    while True:
        ...
        print "[==>] Received inccoming connection from %s:%d" % (addr[0],addr[1])
...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    # now lets loop and read from local and send to remote, send to local
    # rinse, wash ,repeat
    while True:
...
def main():
    ...
    if len(sys.argv[1:]) != 5:
        ...
        print "Example: ./tcp_proxy 127.0.0.1 9000 10.0.0.1 9000 True"
        sys.exit(0)
